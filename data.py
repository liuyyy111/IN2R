...
class PrecompDataset(data.Dataset):
    ...
    def __init__(
        ...
        if data_split == "dev":
            self.length = self.length * self.im_div
    ...
    def __getitem__(self, index):
        ...
        text = process_caption(self.vocab, caption, self.train)

        image = self.images[self.t2i_index[index]]
        # if self.mode == "unlabeled":
            # image = self.images[self._t2i_index[index]]
...
def collate_fn(data):
    ...
    if len(data[0]) == 6:
        return all_images, img_lengths, text, lengths, ids, labels, prob, _labels
    elif len(data[0]) == 5:
        return all_images, img_lengths, text, lengths, ids, _labels
    else:
        return all_images, img_lengths, text, lengths, ids


def get_dataset(data_path, data_name, data_split, return_id_caps=False):
    ...
    # images
    images = np.load(
        os.path.join(data_path, "%s_ims.npy" % data_split), 
        mmap_mode="r"
    )
    print(
    ...
    if return_id_caps:
        # print(captions, images, img_ids, captions)
        return captions, images, img_ids, captions
    else:
...
def get_loader(
    ...
    if data_split == "warmup":
        dset = PrecompDataset(captions, images, vocab, "train", noise_ratio, noise_file)
        ...
        dset = PrecompDataset(captions, images, vocab, data_split)
        data_loader = torch.utils.data.DataLoader(
            dataset=dset,
            batch_size=batch_size,
            shuffle=False,
